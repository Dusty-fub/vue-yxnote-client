...
const actions = {
  ...
  clearTrashNotes({ commit }) {
    return Trash.clearNotes(this.state.trash.trashNotes).then(res => {
      commit("clearTrashNotes");
      res[0] &&
        Message.success(
          res.every(item => item.msg === res[0].msg) ? res[0].msg : "发生异常"
        );
    });
  },
